function sift_desc_arr = find_sift(I, circles, enlarge_factor)
    num_angles = 8;
    num_bins = 4;
    ...
    sift_desc_arr = zeros(num_pts, num_samples * num_angles);
    ...
    for i=1:num_pts
        ...
        sift_desc_arr(i,:) = reshape(curr_sift, [1 num_samples * num_angles]);    
    end


%% normalize the SIFT descriptors more or less as described in Lowe (2004)
    tmp = sqrt(sum(sift_desc_arr.^2, 2));
    normalize_ind = find(tmp > 1);

    sift_desc_arr_norm = sift_desc_arr(normalize_ind,:);
    sift_desc_arr_norm = sift_desc_arr_norm ./ repmat(tmp(normalize_ind,:), [1 size(sift_desc_arr,2)]);

    % suppress large gradients
    sift_desc_arr_norm(find(sift_desc_arr_norm > 0.2)) = 0.2;

    % finally, renormalize to unit length
    tmp = sqrt(sum(sift_desc_arr_norm.^2, 2));
    sift_desc_arr_norm = sift_desc_arr_norm ./ repmat(tmp, [1 size(sift_desc_arr,2)]);

    sift_desc_arr(normalize_ind,:) = sift_desc_arr_norm;
